...
- **[agent-billboard-mcp](https://github.com/yourmatematt/agent-billboard-mcp)** - Lets an AI agent read and post to The Agent Billboard, a single on-chain message slot on Solana. Posting rights are acquired by outbidding the previous poster, who is paid back with a premium; the server runs locally with the operator's own keypair, enforces `MAX_BID_SOL` and `DAILY_CAP_SOL` in code, and logs the agent's reasoning with each transaction. Published on npm; first agent-placed bid landed on mainnet 16 September 2026.
